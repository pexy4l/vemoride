                ...
                <Link key={car.id} to={`/car/${car.id}`} className="bg-white dark:bg-gray-700 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow border border-gray-200 dark:border-gray-600">
                  ...
                  <div className="p-3">
                    ...
                    <div className="flex items-center justify-between mt-1">
                      <span className="text-sm text-brand font-bold">₦{(car.pricing.daily / 1000).toFixed(0)}k/day</span>
                      <span className="flex items-center gap-0.5 text-xs"><Star className="h-3 w-3 fill-yellow-400 text-yellow-400" />{car.rating}</span>
                    </div>
                  </div>
                </Link>
              ))}
      ...
      <section className="py-16 bg-white dark:bg-gray-900">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            <div className="text-center"><div className="w-14 h-14 bg-brand/10 rounded-full flex items-center justify-center mx-auto mb-4"><Shield className="h-7 w-7 text-brand" /></div><h3 className="font-bold dark:text-white mb-2">Trusted Partners</h3><p className="text-sm text-gray-600 dark:text-gray-400">All partners are verified. Every car is inspected before listing.</p></div>
            <div className="text-center"><div className="w-14 h-14 bg-brand/10 rounded-full flex items-center justify-center mx-auto mb-4"><Car className="h-7 w-7 text-brand" /></div><h3 className="font-bold dark:text-white mb-2">Professional Drivers</h3><p className="text-sm text-gray-600 dark:text-gray-400">Every rental comes with a professional, rated driver.</p></div>
            <div className="text-center"><div className="w-14 h-14 bg-brand/10 rounded-full flex items-center justify-center mx-auto mb-4"><Clock className="h-7 w-7 text-brand" /></div><h3 className="font-bold dark:text-white mb-2">Flexible Rentals</h3><p className="text-sm text-gray-600 dark:text-gray-400">Daily, weekly, or monthly. Discounts for longer bookings.</p></div>
          </div>
        </div>
      </section>
